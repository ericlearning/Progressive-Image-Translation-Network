...
class Trainer():
	def __init__(self, loss_type, netD_A, netD_B, netG_A2B, netG_B2A, device, train_ds, val_ds, lr_D = 0.0002, lr_G = 0.0002, cycle_weight = 10, identity_weight = 5.0, ds_weight = 8, resample = True, weight_clip = None, use_gradient_penalty = False, loss_interval = 50, image_interval = 50, save_img_dir = 'saved_images/'):
		self.netD_A = netD_A
		self.netD_B = netD_B
	...
	def train(self, res_num_epochs, res_percentage, bs):
		l1 = nn.L1Loss()
		p, res_percentage = 0, [None] + res_percentage
		...
		for i, (num_epoch, percentage, cur_bs) in enumerate(zip(res_num_epochs, res_percentage, bs)):
			...
			for epoch in range(num_epoch):
				...
				for j, (a, b) in enumerate(tqdm(train_dl)):
					...
					fake_a = self.netG_B2A(b, noise, p)
					...
					cycle_a = self.netG_B2A(fake_b, noise, p)
					cycle_b = self.netG_A2B(fake_a, noise, p)
					identity_a = self.netG_B2A(a, noise, p)
					identity_b = self.netG_A2B(b, noise, p)
					...
					if(self.ds_weight == 0):
						...
						fake_a1 = self.netG_B2A(b, noise1, p)
						fake_a2 = self.netG_B2A(b, noise2, p)
						fake_b1 = self.netG_A2B(a, noise1, p)
						fake_b2 = self.netG_A2B(a, noise2, p)
					...
					if(j % self.loss_interval == 0):
						print('[%d/%d] [%d/%d] errD_A : %.4f, errD_B : %.4f, errG : %.4f'
							  %(epoch+1, num_epoch, j+1, train_dl_len, errD_A, errD_B, errG))

					if(j % self.image_interval == 0):
						if(self.nz == None):
							sample_images_list = get_sample_images_list_pro((val_dl, self.netG_A2B, self.netG_B2A, p, self.device))
							plot_image = get_display_samples(sample_images_list, 6, 3)
						else:
							sample_images_list = get_sample_images_list_noise_pro((val_dl, self.netG_A2B, self.netG_B2A, self.fixed_noise, p, self.device))
