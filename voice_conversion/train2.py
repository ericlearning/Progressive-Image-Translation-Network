...
from architectures.baseline.discriminator import *
from trainers.trainer2 import Trainer
from utils.utils import save_extra, load_extra
...
netD_A_2 = PatchGan_D_286x286_One_Input(ic, use_sigmoid, norm_type, use_sn = False).to(device)
netD_B_2 = PatchGan_D_286x286_One_Input(oc, use_sigmoid, norm_type, use_sn = False).to(device)
...
trainer = Trainer('SGAN', netD_A, netD_B, netD_A_2, netD_B_2, netG_A2B, netG_B2A, device, trn_dl, val_dl, lr_D = lr_D, lr_G = lr_G, cycle_weight = 10, identity_weight = 5.0, ds_weight = 8, resample = True, weight_clip = None, use_gradient_penalty = False, loss_interval = 150, image_interval = 300, save_img_dir = 'saved_imges')

trainer.train(5)
save_extra('saved/cur_state.state', netD_A, netD_B, netD_A_2, netD_B_2, netG_A2B, netG_B2A, trainer.optimizerD_A, trainer.optimizerD_B, trainer.optimizerG)